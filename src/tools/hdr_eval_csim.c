...
int read_CSIM_record(FILE *fp, TREC_Record *T, int width)
{
  char line[1024];
  if (!fgets(line, 1024, fp)) return 0;
  double csim;
  int r = sscanf(line, "%d %s %s %d %d %s %lf", &T->topic, T->q0, T->docname, &T->rank, &T->score, T->runname, &csim);
  T->hd = (1.0 - csim) * width;
  fprintf(stderr, "%f -> %d\n", csim, T->hd);
  //if (r < 6) return 0;
  return 1;
...
{
  if (argc < 4) {
    fprintf(stderr, "Usage: %s [sig width] [truth file] {one or more inputs...}\n", argv[0]);
    ...
    for (int arg = 3; arg < argc; arg++) {
      ...
      for (;;) {
        TREC_Record truth;
        TREC_Record input;
        if (!read_CSIM_record(ft, &truth, atoi(argv[1])) || !read_TREC_record(fi, &input)) eof = 1;
