...
{
  char sig_method[64];
  cfg.headersize = file_read32(fi); // header-size
  int version = file_read32(fi); // version
  cfg.maxnamelen = file_read32(fi); // maxnamelen
  cfg.sig_width = file_read32(fi); // sig_width
  file_read32(fi); // sig_density
  if (version >= 2) {
    file_read32(fi); // sig_seed
  }
